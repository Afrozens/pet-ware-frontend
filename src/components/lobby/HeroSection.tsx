        ...
        <div className="w-full flex flex-col gap-5 items-start">
          <div className="mb-5 text-3xl xl:text-4xl 2xl:text-6xl font-semibold text-white space-y-3 md:space-y-4 lg:space-y-5">
            <span className="block">
              <ColourText text={t('professionals')} /> {t('for')}
            </span>
            <span className="block">
              {t('you')} <ColourText text={t('pet')} />,
            </span>
            <span className="block">
              {t('directly')} <ColourText text={t('home')} />
            </span>
          </div>
          <div
            className={`text-xl xl:text-2xl w-full text-gray-400 ${montserrat.className}`}
          >
            <span className="block w-full">
              {t('find-verified')}{' '}
              <FlipText words={['Puerto Ordaz', 'San Felix', 'Lechería', 'Chacao']} />
            </span>
            <span className="block">{t('services')}</span>
            <span className="block">{t('one-place')}.</span>
          </div>
          <div className="w-full max-w-2xl flex gap-10 justify-between items-center">
            <ButtonPrimary color="bg-success-pressed" small={false} withIcon={false}>
              <span className="text-2xl text-white">{t('search-professional')}</span>
            </ButtonPrimary>
            <ButtonPrimary color="bg-white" small={false} withIcon={false}>
              <span className="text-2xl text-black">{t('i-professional')}</span>
            </ButtonPrimary>
          </div>
        </div>
